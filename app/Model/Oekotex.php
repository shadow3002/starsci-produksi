 ...
 *
 */
class Oekotex extends AppModel {
 ...
 */
	...
	public function namaproduk($data = array()){
		...
		$cond = array(
		'AND' =>array(
		'Oekotex.name LIKE' => '%'.$data['namaproduk'].'%',
		'OekotexCategory.name LIKE' => '%'.$data['category'].'%'));
		return $cond;
		}
		else{
		if($data['namaproduk']){
	...
	'OekotexCategory'=> array(
			'className' => 'OekotexCategory',
			'foreignKey' => 'oekotex_category_id'
			)
	);
