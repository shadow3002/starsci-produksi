 ...
 */
	...
	public function namaproduk($data = array()){
		...
		$cond = array(
		'AND' =>array(
		'Tdse.name LIKE' => '%'.$data['namaproduk'].'%',
		'TdsCategory.name LIKE' => '%'.$data['category'].'%'));
		return $cond;
		}
		else{
		if($data['namaproduk']){
	...
	'TdsCategory'=> array(
			'className' => 'TdsCategory',
			'foreignKey' => 'tds_category_id'
			)
	);
