 ...
 */
	...
	public function namaproduk($data = array()){
		...
		$cond = array(
		'AND' =>array(
		'Ikea.name LIKE' => '%'.$data['namaproduk'].'%',
		'IkeaCategory.name LIKE' => '%'.$data['category'].'%'));
		return $cond;
		}
		else{
		if($data['namaproduk']){
	...
	'IkeaCategory'=> array(
			'className' => 'IkeaCategory',
			'foreignKey' => 'ikea_category_id'
			)
	);
